...
class AppPreferences(context: Context) {
    ...
    /**
     * Tiny synchronous mirror of the few preferences that are needed *before*
     * the first frame (app language, hide-from-recents, routing mode).
     *
     * Reading those from DataStore inside `Activity.attachBaseContext()` /
     * `onCreate()` required `runBlocking` on the main thread — a blocking disk
     * read in the middle of app launch. A plain `SharedPreferences` read is
     * served from an in-memory map, so it costs microseconds.
     */
    ...
    // ── Synchronous launch-path cache ────────────────────────────────────
    // These are safe to call from the main thread: the value normally comes
    // straight from memory. Only the very first launch after an install or an
    // upgrade (when the mirror is still empty) falls back to DataStore once and
    // then caches the result.

    fun getAppLanguageCached(): String {
        fastPrefs.getString(KEY_APP_LANGUAGE, null)?.let { return it }
        return runBlocking { appearance.appLanguage.first() }.also {
            fastPrefs.edit().putString(KEY_APP_LANGUAGE, it).apply()
        }
    }

    fun getHideFromRecentsCached(): Boolean {
        if (fastPrefs.contains(KEY_HIDE_FROM_RECENTS)) {
            return fastPrefs.getBoolean(KEY_HIDE_FROM_RECENTS, false)
        }
        return runBlocking { vpnSecurity.hideFromRecents.first() }.also {
            fastPrefs.edit().putBoolean(KEY_HIDE_FROM_RECENTS, it).apply()
        }
    }

    fun getRoutingModeCached(): String {
        fastPrefs.getString(KEY_ROUTING_MODE, null)?.let { return it }
        return runBlocking { wireguard.getRoutingModeSnapshot() }.also {
            fastPrefs.edit().putString(KEY_ROUTING_MODE, it).apply()
        }
    }
    ...
    suspend fun setHideFromRecents(enabled: Boolean) {
        vpnSecurity.setHideFromRecents(enabled)
        fastPrefs.edit().putBoolean(KEY_HIDE_FROM_RECENTS, enabled).apply()
    }
    ...
    suspend fun setAppLanguage(language: String) {
        appearance.setAppLanguage(language)
        fastPrefs.edit().putString(KEY_APP_LANGUAGE, language).apply()
    }
    ...
    suspend fun setRoutingMode(mode: String) {
        ...
        fastPrefs.edit().putString(KEY_ROUTING_MODE, mode).apply()
    }
