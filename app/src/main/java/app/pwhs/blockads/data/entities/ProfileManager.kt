...
class ProfileManager(
    ...
    suspend fun seedPresetsIfNeeded() = withContext(Dispatchers.IO) {
        ...
        val presets = listOf(
            ...
            ProtectionProfile(
                ...
                safeSearchEnabled = true,
                youtubeRestrictedMode = true,
                // Maximum blocking is the default out of the box.
        ...
        // Activate the strongest preset on first launch so protection is at its
        // maximum without any manual configuration.
        if (existing.isEmpty()) {
            val allProfiles = profileDao.getAllSync()
            val strictFamilyProfile = allProfiles.firstOrNull {
                it.profileType == ProtectionProfile.TYPE_STRICT_FAMILY
            }
            if (strictFamilyProfile != null) {
                // Use switchToProfile so filters and preferences are consistent
                switchToProfile(strictFamilyProfile.id)
            }
        }
    }
    ...
    suspend fun switchToProfile(profileId: Long) = withContext(Dispatchers.IO) {
        ...
        // The "Strict Family" preset means maximum blocking: every available
        // list is enabled, regardless of whether the remote manifest URLs
        // happen to match the hard-coded preset set.
        val enableEverything = targetProfile.profileType == ProtectionProfile.TYPE_STRICT_FAMILY
        for (filter in allFilters) {
            val shouldBeEnabled = enableEverything || filter.url in targetUrls
