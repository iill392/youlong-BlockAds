...
interface DnsLogDao {
    ...
    /**
     * Batched counterpart of [insert]. The engine produces one entry per DNS
     * query; inserting them one-by-one invalidated the whole table that many
     * times per second and starved every other query on the shared Room
     * executor. Batching keeps both the write cost and the invalidation rate
     * proportional to time, not to query volume.
     */
    @Insert(onConflict = OnConflictStrategy.REPLACE)
    ...
    /**
     * Retention: drop entries older than [cutoff]. Without this the table grew
     * forever (it was only ever cleared en bloc), which made every Home-screen
     * aggregate steadily slower and, because those aggregates run on the shared
     * Room executor, delayed service start-up too.
     */
    @Query("DELETE FROM dns_logs WHERE timestamp < :cutoff")
    suspend fun deleteOlderThan(cutoff: Long): Int

    /** Retention: hard cap on table size, keeping the most recent [keep] rows. */
    @Query("DELETE FROM dns_logs WHERE id NOT IN (SELECT id FROM dns_logs ORDER BY timestamp DESC LIMIT :keep)")
