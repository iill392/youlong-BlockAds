    ...
    private fun startProxy(startedFromBoot: Boolean = false) {
        ...
        serviceScope.launch {
            try {
                ...
                //    Network-free, exactly like VpnEngineCoordinator.prepareStartupConfig():
                //    remote manifest sync + missing binary downloads are deferred to
                //    backgroundSyncMissingFilters() so startup is never blocked by a
                //    slow or unreachable filter CDN.
                filterRepo.loadWhitelist()
                filterRepo.loadCustomRules()
                val result = filterRepo.loadAllEnabledFilters(offlineOnly = true)
                Timber.d("Filters loaded for Root Proxy mode: ${result.getOrDefault(0)} domains")

                // Same guarantee as the VPN path: never leave the engine with
                // no rules at all — but run it detached, because the recovery is
                // network-bound and may take minutes. Blocking here would stall
                // the whole Root Proxy start-up on the filter CDN.
                if (!filterRepo.hasUsableRules()) {
                    serviceScope.launch { filterRepo.ensureUsableRules() }
                }

                // Fetch anything still missing in the background, then make
                // sure the engine actually re-reads its rule paths.
                serviceScope.launch {
                    filterRepo.backgroundSyncMissingFilters()
                    goTunnelAdapter.updateTries()
                    goTunnelAdapter.updateCosmeticRules()
                    if (!filterRepo.hasUsableRules()) {
                        Timber.e("Root Proxy is running without any blocklist — filter CDN unreachable and no cached rules")
                    }
                }

                // Hot-reload rules whenever the rule set changes.
                serviceScope.launch {
                    filterRepo.filtersRevision.drop(1).collectLatest { revision ->
                        Timber.d("Filter rules changed (revision=$revision) — updating Root Proxy tries")
                        goTunnelAdapter.updateTries()
                        goTunnelAdapter.updateCosmeticRules()
                    }
                }
