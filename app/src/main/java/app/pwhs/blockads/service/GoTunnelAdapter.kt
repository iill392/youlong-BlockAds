...
class GoTunnelAdapter(
    ...
    // ── Batched DNS log pipeline ────────────────────────────────────────
    //
    // The engine invokes the log callback for *every* DNS query. The previous
    // implementation started a fresh coroutine per query, did a PackageManager
    // binder round trip per query and wrote a **single** row per query. That
    // hammered the shared Room executor and invalidated the `dns_logs` table
    // hundreds of times a second, which in turn made the Home screen's seven
    // aggregate queries re-run non-stop and starved the VPN service of the
    // database while it was starting up — the "sometimes instant, sometimes
    // forever" behaviour users reported.
    //
    // Events are now buffered and written in batches by a single drainer, and
    // app labels are memoised so the binder call happens once per app.
    private val logQueue = ArrayBlockingQueue<LogEvent>(LOG_QUEUE_CAPACITY)
    private val appLabelCache = ConcurrentHashMap<String, String>()

    @Volatile
    private var droppedLogEvents = 0

    private var logDrainer: Job? = null
    ...
    private data class LogEvent(
        ...
        val packageNameOrAppName: String,
     ...
     *
     * The callback only enqueues: it must stay allocation-light and I/O-free
     * because the native engine calls it for every single DNS query.
     */
    private fun setupLogCallback() {
        ...
        engine.setLogCallback { domain, blocked, queryType, responseTimeMs, packageNameOrAppName, resolvedIP, blockedBy ->
            ...
            val accepted = logQueue.offer(
                LogEvent(
                    domain = domain,
                    blocked = blocked,
                    queryType = dnsQueryTypeToString(queryType.toInt()),
                    responseTimeMs = responseTimeMs,
                    packageNameOrAppName = packageNameOrAppName,
    ...
    private fun startLogDrainer() {
        if (logDrainer?.isActive == true) return
        logDrainer = scope.launch(Dispatchers.IO) {
            val events = ArrayList<LogEvent>(LOG_BATCH_SIZE)
            while (isActive) {
                try {
                    val first = logQueue.poll(LOG_FLUSH_INTERVAL_MS, TimeUnit.MILLISECONDS)
                        ?: continue
                    events.clear()
                    events.add(first)
                    logQueue.drainTo(events, LOG_BATCH_SIZE - 1)

                    dnsLogDao.insertAll(events.map(::toEntry))

                    val dropped = droppedLogEvents
                    if (dropped > 0) {
                        droppedLogEvents = 0
                        Timber.w("Dropped $dropped DNS log entries (buffer full)")
                    }
                } catch (_: InterruptedException) {
                    return@launch
                } catch (e: Exception) {
                    Timber.e(e, "Failed to persist DNS log batch")
                    delay(LOG_RETRY_DELAY_MS)
                }
            }
        }
    }

    private fun toEntry(event: LogEvent): DnsLogEntry {
        val label = if (event.packageNameOrAppName.contains('.')) {
            appLabelCache.getOrPut(event.packageNameOrAppName) {
                try {
                    val pm = context.packageManager
                    val info = pm.getApplicationInfo(event.packageNameOrAppName, 0)
                    pm.getApplicationLabel(info).toString()
                } catch (_: Exception) {
                    event.packageNameOrAppName
                }
            }
        } else {
            event.packageNameOrAppName
        ...
        return DnsLogEntry(
            ...
            appName = label,
            packageName = event.packageNameOrAppName,
    ...
    fun start(
        ...
        val adTrieCount = filterRepo.getAdTriePath().split(',').count { it.isNotBlank() }
        val adBloomCount = filterRepo.getAdBloomPath().split(',').count { it.isNotBlank() }
        val secTrieCount = filterRepo.getSecurityTriePath().split(',').count { it.isNotBlank() }
        val secBloomCount = filterRepo.getSecurityBloomPath().split(',').count { it.isNotBlank() }
        Timber.d(
            "Engine rule sources: adTrie=$adTrieCount adBloom=$adBloomCount " +
                "securityTrie=$secTrieCount securityBloom=$secBloomCount " +
                "(usable=${filterRepo.hasUsableRules()})"
        )
        if (!filterRepo.hasUsableRules()) {
            Timber.e("Starting engine with NO rule sources — DNS filtering will not block anything")
        }
        updateTries()
        updateCosmeticRules()
    ...
    companion object {
        private const val LOG_QUEUE_CAPACITY = 4096
        private const val LOG_BATCH_SIZE = 200
        private const val LOG_FLUSH_INTERVAL_MS = 500L
        private const val LOG_RETRY_DELAY_MS = 1_000L
